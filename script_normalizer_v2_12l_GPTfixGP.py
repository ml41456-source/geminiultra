# ===========================================================
# script_normalizer_v2_12l.py
# Screenplay Normalizer (VI/EN) → StoryGrid v2.12l
#
# Nâng cấp trọng điểm:
#  - Garment/Color: neo theo owner bên trái, KHÔNG fallback last_subject.
#  - Bỏ "khăn" (từ chung) khỏi garment; chỉ bind khi có wear-verb hoặc là "khăn quàng"/"nơ".
#  - Negation guard trong mine_actions_v2.
#  - Presence pass: thêm nhân vật chỉ-được-nhắc-đến.
#  - Mở rộng COLOR_CANON & PRODUCE_SYNONYMS.
#  - FULLNAME_RE động từ alias_map (hỗ trợ đa kịch bản).
#  - Giữ debug/verbose & QC log.
# ===========================================================
...
# Garment words: BỎ "khăn" chung để tránh false positive
GARMENT_WORDS = [
...
STRONG_ACTIONS = {
    "nhún nhảy","vươn vai","chạy","ôm","ôm chầm","giơ lên","giơ tay","chỉ","chỉ vào","trỏ",
    "jumps","stretches","runs","hug","hug tight","raise","point","point to","dash","throw","snatch","pick up",
    "dậm","dậm chân","vồ","vơ","nhặt"
}
CANON_ACTION = {
...
# -------------------- Garment/Color binding theo owner --------------------
...
def bind_attrs_in_sentence(sentence:str, alias_map:Dict[str,str], chars:Dict[str,Dict], last_subject:str=None):
    ...
    # Garment + Color: bind theo owner (không dùng last_subject)
...
def seed_variants_from_text(scene_text:str, chars:Dict[str,Dict]):
    ...
    if m:
        ...
        col = "xanh_la" if "xanh" in col_raw else "xanh"
...
def extract_characters(scene_text:str, alias_map:Dict[str,str], issues:List[str], sid:int)->Dict[str,Dict]:
    ...
    for nm in mentions_all:
        sl = slugify(nm)
        if sl not in chars:
            chars[sl] = {"role":"student","attributes":["present"], "aliases":[], "confidence":0.22}
...
def normalize_script(script_path:str, output_dir:str="./output_normalized"):
    ...
    # FULLNAME_RE động theo alias_map
    global FULLNAME_RE
    ...
    for sc in scenes:
        ...
        def sort_key(v):
            ...
            return (0 if role=="teacher" else 1, -v.get("Confidence",0.0))
        char_list = [ {
        ...
        scene_entry["Characters"] = sorted(char_list, key=sort_key)
